 ...
 * from that token's JWT `sub` claim. Wishlist read/write use the store endpoints directly.
 */
class SteamConnector : ServiceConnector(
    ...
    override fun readWishlist() = WebStep(HOME, SyncScripts.wrap("""
        let r = await fetch('https://store.steampowered.com/dynamicstore/userdata/', { credentials: 'include' });
        let j = await r.json();
        out = (j.rgWishlist || []).map(id => ({ uid: String(id), name: '' }));
    """.trimIndent()))
